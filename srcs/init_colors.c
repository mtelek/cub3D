...
/*   Updated: 2024/11/19 18:13:04 by mtelek           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../Headers/cub3D.h"

int	rgb_to_hex(const char *rgb_str, int *r, int *g, int *b, t_main *main)
{
	char	*token;
	char	*rgb_copy;
...
void	init_floor_and_ceiling_colors(t_textures *textures, t_main *main)
{
	...
	b = -1;
	if (textures->floor)
		textures->floor_color = rgb_to_hex(textures->floor, &r, &g, &b, main);
	if (textures->ceiling)
