...
/*   Updated: 2024/10/22 20:19:22 by mtelek           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void get_display_resolution(t_main *main)
{
    ...
    if (fd == -1)
    {
        printf("Failed to open framebuffer virtual_size. Falling back to 800x600\n");
        main->s_width = 800;
        main->s_height = 800;
        return;
    }
    ...
    {
        printf("Could not read resolution. Falling back to 800x600\n");
        main->s_width = 800;
        main->s_height = 800;
    }
    if (close(fd) == -1)
